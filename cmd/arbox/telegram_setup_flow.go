...
func handleTelegramSetup(ctx context.Context, hc *http.Client, base string, chatID, replyTo int64, cfg *config.Config, client *arboxapi.Client, locID int) error {
	...
	headLines := []string{
		...
		notify.EscapeMarkdownV2("  ✓  selected — daemon will try to book this slot"),
		notify.EscapeMarkdownV2("  ○  off — not booked (a day with no ✓ becomes a rest day)"),
		"",
		notify.EscapeMarkdownV2("One pick per day (tap a different ○ to replace; tap the ✓ again to clear)."),
		notify.EscapeMarkdownV2("Existing ✓ are seeded from your saved plan."),
		"",
